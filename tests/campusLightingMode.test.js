...
test('derives a restrained evening state with stronger operational lights', async () => {
  ...
  assert.equal(day.skyColor, 0x88adba, 'day mode needs a visible blue-gray architectural sky instead of a navy void')
  ...
  assert.equal(day.hemisphereSkyColor, 0xeaf4f2)
  assert.equal(day.hemisphereGroundColor, 0x879080)
  assert.equal(day.keyColor, 0xffdfb8)
  assert.equal(day.fillColor, 0xc7dce2)
  ...
  assert.equal(evening.exposure, 1.18, 'evening mode must retain enough facade detail for the enlarged campus')
  assert.equal(evening.skyColor, 0x071926)
  assert.equal(evening.fogColor, 0x163747)
  assert.equal(evening.environmentRotation, -.08)
  assert.ok(evening.ambientIntensity >= 2.1, 'evening mode needs readable building silhouettes around the functional lights')
  ...
  assert.equal(evening.keyIntensity + evening.shadowFillIntensity, 1.92, 'evening key split must preserve the approved light energy')
})

test('resolves a layered architectural horizon for both lighting modes', async () => {
  const module = await import('../src/scene/campusLightingMode.js').catch(() => ({}))
  assert.equal(typeof module.resolveCampusBackdrop, 'function', 'layered campus backdrop resolver is missing')

  const day = module.resolveCampusBackdrop('day')
  const evening = module.resolveCampusBackdrop('evening')
  assert.deepEqual(day, { topColor: 0x88adba, horizonColor: 0xb8d2d4, lowColor: 0x6f8778 })
  assert.deepEqual(evening, { topColor: 0x071926, horizonColor: 0x214657, lowColor: 0x101c22 })
  assert.notEqual(evening.topColor, evening.horizonColor, 'blue-hour horizon needs visible depth')
})

test('keeps the layered backdrop centered on the moving camera and inside its far plane', async () => {
  const module = await import('../src/scene/campusLightingMode.js').catch(() => ({}))
  assert.equal(typeof module.updateCampusBackdropCamera, 'function', 'camera-following backdrop updater is missing')
  const scene = new THREE.Scene()
  const camera = new THREE.PerspectiveCamera(38, 1, .1, 160)
  camera.position.set(-47, 29, -58)

  module.applyCampusLightingMode({ scene, camera, renderer: { toneMappingExposure: 1 } }, 'evening')
  const backdrop = scene.getObjectByName('CampusBackdrop')
  assert.deepEqual(backdrop.position.toArray(), camera.position.toArray())
  assert.ok(backdrop.scale.x < camera.far, 'the visible sky surface must remain before the far clipping plane')

  camera.position.set(32, 24, 46)
  module.updateCampusBackdropCamera(scene, camera)
  assert.deepEqual(backdrop.position.toArray(), camera.position.toArray())
  assert.ok(backdrop.scale.x <= camera.far * .84)
})
...
test('applies lighting mode to the renderer, fog, rig, and emissive site fixtures', async () => {
  ...
  assert.equal(scene.background.getHex(), 0x88adba)
  assert.equal(hemisphere.color.getHex(), 0xeaf4f2)
  assert.equal(hemisphere.groundColor.getHex(), 0x879080)
  assert.equal(key.color.getHex(), 0xffdfb8)
  assert.ok(key.intensity > fill.intensity * 2)
  assert.equal(fill.color.getHex(), 0xc7dce2)
  ...
  assert.equal(renderer.toneMappingExposure, 1.18)
  assert.ok(scene.environmentIntensity < 1)
  assert.equal(scene.environmentRotation.y, -.08)
  assert.equal(scene.fog.color.getHex(), 0x163747)
  const backdrop = scene.getObjectByName('CampusBackdrop')
  assert.ok(backdrop?.isMesh, 'lighting mode should maintain one layered sky mesh')
  assert.equal(backdrop.material.uniforms.horizonColor.value.getHex(), 0x214657)
  assert.ok(road.intensity > 1)
  assert.ok(fixture.material.emissiveIntensity > 1)
  assert.deepEqual(modeCalls, [['day', 1.32], ['evening', 1.18]])
})
