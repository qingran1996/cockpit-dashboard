...
export function EnergyDetailSidecar({ resource, detail, activeTab, onTabChange, onResourceChange, onClose }) {
  ...
  h('section', { className: 'energy-detail__return-rail', 'aria-label': '二级页面导航' },
    h('div', { className: 'energy-detail__return-context' },
      h('small', null, 'ENERGY DETAIL'),
      h('strong', null, '运行总览'),
    ),
    h('button', { type: 'button', className: 'energy-detail__return-action', onClick: onClose, 'aria-label': '返回能源首页' },
